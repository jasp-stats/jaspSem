...
test_that("Factor Loadings table results match", {
  table <- results[["results"]][["modelContainer"]][["collection"]][["modelContainer_params"]][["collection"]][["modelContainer_params_ind"]][["data"]]
  expect_bsem_table(table,
                                 list(1, 1, 1, 1, "", "ind60", "", "x1", 0, 2.0187451982173, 2.6240399552204,
                                      2.28454934540751, 1, "", "ind60", "normal(0,10)", "x2", 0.157731909136992,
                                      1.60238526900487, 2.2519794889017, 1.90212156515777, 1, ".p3.",
                                      "ind60", "normal(0,10)", "x3", 0.176424180962907, 1, 1, 1, 1,
                                      "", "dem60", "", "y1", 0, 1.22631886860213, 2.17386546821108,
                                      1.6411725419051, 1, ".p5.", "dem60", "normal(0,10)", "y2", 0.238928904485913,
                                      0.75418376271223, 1.47957008581177, 1.1028702249245, 1, ".p6.",
                                      "dem60", "normal(0,10)", "y3", 0.186245869844387, 1.22675988372089,
                                      1.93766716227724, 1.54326085137414, 1, ".p7.", "dem60", "normal(0,10)",
                                      "y4", 0.196657176039281, 1, 1, 1, 2, "", "ind60", "", "x1",
                                      0, 1.90788696995042, 2.94262248630884, 2.40612695405118, 2,
                                      "", "ind60", "normal(0,10)", "x2", 0.259912653204619, 1.60238526900487,
                                      2.2519794889017, 1.90212156515777, 2, ".p3.", "ind60", "", "x3",
                                      0.176424180962907, 1, 1, 1, 2, "", "dem60", "", "y1", 0, 1.22631886860213,
                                      2.17386546821108, 1.6411725419051, 2, ".p5.", "dem60", "", "y2",
                                      0.238928904485913, 0.75418376271223, 1.47957008581177, 1.1028702249245,
                                      2, ".p6.", "dem60", "", "y3", 0.186245869844387, 1.22675988372089,
                                      1.93766716227724, 1.54326085137414, 2, ".p7.", "dem60", "",
                                      "y4", 0.196657176039281))
})

test_that("Factor Variances table results match", {
  table <- results[["results"]][["modelContainer"]][["collection"]][["modelContainer_params"]][["collection"]][["modelContainer_params_lvar"]][["data"]]
  expect_bsem_table(table,
                                 list(0.252315347895461, 0.711065489854395, 0.442556791853438, 1, "",
                                      "ind60", "gamma(1,0.5)[sd]", 0.123327970575969, 1.36265611096552,
                                      5.07166468028354, 2.84439368096978, 1, "", "dem60", "gamma(1,0.5)[sd]",
                                      0.945393247703378, 0.186107751161238, 0.647923797397383, 0.34977159675042,
                                      2, "", "ind60", "gamma(1,0.5)[sd]", 0.10795111845038, 1.99938100275197,
                                      6.2981414205291, 3.73052581659, 2, "", "dem60", "gamma(1,0.5)[sd]",
                                      1.22430001797541))
})
...
test_that("Regression Coefficients table results match", {
  table <- results[["results"]][["modelContainer"]][["collection"]][["modelContainer_params"]][["collection"]][["modelContainer_params_reg"]][["data"]]
  expect_bsem_table(table,
                                 list(0.804264151935036, 2.72451401211712, 1.70033246756807, 1, "",
                                      "dem60", "normal(0,10)", "ind60", 0.463396456444412, -0.042703890140392,
                                      2.18124118541734, 0.990204584279713, 2, "", "dem60", "normal(0,10)",
                                      "ind60", 0.599136180300971))
})

test_that("Residual Variances table results match", {
  table <- results[["results"]][["modelContainer"]][["collection"]][["modelContainer_params"]][["collection"]][["modelContainer_params_var"]][["data"]]
  expect_bsem_table(table,
                                 list(0.031340125699617, 0.103461197039765, 0.0597237007744274, 1, "",
                                      "x1", "gamma(1,0.5)[sd]", 0.0178895486359223, 1.95925269844761e-05,
                                      0.186197764719793, 0.0441624918117041, 1, "", "x2", "gamma(1,0.5)[sd]",
                                      0.0538502951471386, 0.3367222705469, 0.874222489645218, 0.551002978304832,
                                      1, "", "x3", "gamma(1,0.5)[sd]", 0.135154633836422, 2.05408256447058,
                                      5.89755647692826, 3.58855643403319, 1, "", "y1", "gamma(1,0.5)[sd]",
                                      0.985325318586375, 3.05977481054924, 9.23286225779755, 5.35383088583362,
                                      1, "", "y2", "gamma(1,0.5)[sd]", 1.51666615924359, 3.65749234125426,
                                      10.930125789774, 6.23682318694169, 1, "", "y3", "gamma(1,0.5)[sd]",
                                      1.77366941909075, 3.44010303519335e-05, 1.74507948155232, 0.457620683319961,
                                      1, "", "y4", "gamma(1,0.5)[sd]", 0.526582724946732, 0.0437570684597088,
                                      0.177963912458444, 0.0979761770175269, 2, "", "x1", "gamma(1,0.5)[sd]",
                                      0.0328091834779875, 0.0100524822500044, 0.529707367741576, 0.212581342555664,
                                      2, "", "x2", "gamma(1,0.5)[sd]", 0.143022582329954, 0.269044475496772,
                                      0.905521587409968, 0.492837887925516, 2, "", "x3", "gamma(1,0.5)[sd]",
                                      0.148106690594019, 0.992137782074208, 3.79737686381855, 2.17301469570636,
                                      2, "", "y1", "gamma(1,0.5)[sd]", 0.755077660644045, 3.00534028528002,
                                      9.93243793408708, 5.96340689059146, 2, "", "y2", "gamma(1,0.5)[sd]",
                                      1.73349283093407, 4.1123768242048, 9.76672305837356, 6.42986726583934,
                                      2, "", "y3", "gamma(1,0.5)[sd]", 1.53477286204819, 1.12648808468416,
                                      6.11329923612775, 3.22516479093823, 2, "", "y4", "gamma(1,0.5)[sd]",
                                      1.31320879833285))
})
